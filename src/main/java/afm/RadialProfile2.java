package afm;
import ij.*;
import ij.plugin.filter.PlugInFilter;
import ij.process.*;
import ij.gui.*;
import ij.measure.Calibration;
import java.awt.*;
import java.util.*;



public class RadialProfile2 {
	//ImagePlus imp;
	boolean canceled=false;
	double X0;
	double Y0;
	double mR;
	Rectangle rct;
	int nBins=100;
	//static boolean doNormalize = true;
	static boolean useCalibration = false;
	PlotWindow pw;

	
	float nBinmax2;

	private void doRadialDistribution(ImageProcessor ip, ImagePlus impraw) {
		nBins = (int) (3*mR/4);
		int thisBin;
		float[][] Accumulator = new float[2][nBins];
		double R;
		double xmin=X0-mR, xmax=X0+mR, ymin=Y0-mR, ymax=Y0+mR;
		for (double i=xmin; i<xmax; i++) {
			for (double j=ymin; j<ymax; j++) {
				R = Math.sqrt((i-X0)*(i-X0)+(j-Y0)*(j-Y0));
				thisBin = (int) Math.floor((R/mR)*(double)nBins);
				if (thisBin==0) thisBin=1;
				thisBin=thisBin-1;
				if (thisBin>nBins-1) thisBin=nBins-1;
				Accumulator[0][thisBin]=Accumulator[0][thisBin]+1;
				Accumulator[1][thisBin]=Accumulator[1][thisBin]+ip.getPixelValue((int)i,(int)j);
			}
		}
		Calibration cal = impraw.getCalibration();
		if (cal.getUnit() == "pixel") useCalibration=false;
		if (useCalibration) {
			for (int i=0; i<nBins;i++) {
				Accumulator[1][i] =  Accumulator[1][i] / Accumulator[0][i];
				Accumulator[0][i] = (float)(cal.pixelWidth*mR*((double)(i+1)/nBins));
			}
//			pw = new PlotWindow("Radial Profile Plot", "Radius ["+cal.getUnits()+"]", "Normalized Integrated Intensity",  Accumulator[0], Accumulator[1]);
		} else {
			for (int i=0; i<nBins;i++) {
				Accumulator[1][i] = Accumulator[1][i] / Accumulator[0][i];
				Accumulator[0][i] = (float)(mR*((double)(i+1)/nBins));
			}
//			pw = new PlotWindow("Radial Profile Plot", "Radius [pixels]", "Normalized Integrated Intensity",  Accumulator[0], Accumulator[1]);
		}
//		pw.draw();

		float max=0;
		float nBinmax=0;
		for(int ii=0;ii<nBins;ii=ii+1)
			if(Accumulator[1][ii]>max)
			{max=Accumulator[1][ii]; nBinmax=Accumulator[0][ii];}
		IJ.log ("Max="+max + " Rad="+ nBinmax);
		nBinmax2=nBinmax;
		


	}

	private void doDialog() {
		canceled=false;
		GenericDialog gd = new GenericDialog("Radial Distribution...", IJ.getInstance());
		gd.addNumericField("X center (pixels):",X0,2);
		gd.addNumericField("Y center (pixels):", Y0,2);
		gd.addNumericField("Radius (pixels):", mR,2);
		//gd.addCheckbox("Normalize", doNormalize);
		gd.addCheckbox("Use Spatial Calibration", useCalibration);
		gd.showDialog();
		if (gd.wasCanceled()) {
			canceled = true;
			return;
		}
		X0=gd.getNextNumber();
		Y0=gd.getNextNumber();
		mR=gd.getNextNumber();
		//doNormalize = gd.getNextBoolean();
		useCalibration = gd.getNextBoolean();
		if(gd.invalidNumber()) {
			IJ.showMessage("Error", "Invalid input Number");
			canceled=true;
			return;
		}
	}


	public void run(ImageProcessor raw, ImagePlus impraw, ImageProcessor ref, ArrayList peaklist, AFMImage afmimage, double convert) {
		
		ImageProcessor img1 = afmimage.imageFromPeak(raw, ref, (Peaks) (peaklist.get(1)), ref.getWidth() /2);
		X0 = (double)img1.getWidth() /2;
		Y0 = (double)img1.getHeight() /2;
		mR =  (img1.getWidth() + img1.getHeight())/4.0;
		nBins = (int) (3*mR/4);
		
		float[] maxHisto = new float[nBins];
		for (int i=0; i<nBins; i++) {
			maxHisto[i] = 0;
		}
		float[] scale = new float[nBins];
		for (int i=0; i<nBins; i++) {
			scale[i] = (float)(mR*((double)(i+1)/nBins));
		}
		
		for (int i = 0; i <peaklist.size(); i++) {
			nBinmax2=0;
			ImageProcessor img = afmimage.imageFromPeak(raw, ref, (Peaks) (peaklist.get(i)), ref.getWidth() /2);
		
			IJ.makeOval((int)(X0-mR), (int)(Y0-mR), (int)(2*mR), (int)(2*mR));
			impraw.startTiming();
			if (canceled) return;
			doRadialDistribution(img, impraw);
			
			for (int j=0; j<nBins; j++) {
				if (nBinmax2==scale[j]) { maxHisto[j]=maxHisto[j]+1;}	
			}
		}
		//change radius [pixel] to radius [Angstrom]
		for (int i=0; i<nBins; i++) {
			scale[i] = scale[i]*(float)convert;
		}
		PlotWindow plot = new PlotWindow("histogram maxima in radial profile plot", "radius [Angstrom]", "number of images", scale, maxHisto);
		plot.draw();
	}
	

}


