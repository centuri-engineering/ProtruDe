...
//	}
	...
	public void run(ImageProcessor ip, ImagePlus impraw) {
		rct = impraw.getRoi().getBounds();
		X0 = (double)rct.x+(double)rct.width/2;
		Y0 =  (double)rct.y+(double)rct.height/2;
		mR =  (rct.width+rct.height)/4.0;
		...
		impraw.startTiming();
		if (canceled) return;
		doRadialDistribution(ip, impraw);

	}

// img = imageFromPeak(raw, ref, (Peaks) (peakList.elementAt(i)), 2*w);	
